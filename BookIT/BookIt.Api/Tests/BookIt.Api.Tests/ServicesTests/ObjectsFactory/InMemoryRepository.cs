...
namespace Bookit.Api.Tests.ServicesTests.ObjectsFactory
{
    public class InMemoryRepository<T> : IRepository<T> where T : class
    {
        ...
        {
            var itemToDelete = this.GetById(id);

            if(itemToDelete == null)
            {
                throw new ArgumentException("There is no entity with that Id");
            }

            if (!this.data.Contains(itemToDelete))
            {
                throw new InvalidOperationException("Entity to delete is missing!");
            }
